...
uint8_t UartMcuPutBuffer( Uart_t *obj, uint8_t *buffer, uint16_t size )
{
    ...
    else
    {
        ...
        for( i = 0; i < size; i++ )
        {
            n = 300;                    // delay little bit when use uart too frequently
            retryCount = 0;
...
uint8_t UartMcuGetFrame( Uart_t *obj, uint8_t *buffer, uint16_t *nbReadBytes )
{
    uint8_t data;
    uint16_t size = 0;
    while( !IsFifoEmpty( &obj->FifoRx ) ) {
        if( !UartGetChar( obj, &data ) ) {
            buffer[size++] = data;
        } else {
            return 1;
        }
...
void USART1_IRQHandler( void )
{
    ...
    // [BEGIN] Add flag when receive one complete frame using idle interrupt
